...
<div class="dashboard-grid overflow-hidden h-full p-5">
    ...
    <div class="overflow-y-scroll left rounded-lg bg-azzurro px-3" style="grid-area: access-log">
        ...
        <div class="">
            ...
            {/each}
            {#if page!==count}
            <button on:click={loadMore} class="bg-black p-3 rounded-2xl mb-3 text-white w-full hover:bg-white hover:text-black group transition-all ease-in-out duration-300 cursor-pointer">
    ...
    <div class="hidden md:block bg-azzurro rounded-lg p-5" style="grid-area: info-card">
        {#if user != null && user_id != "0"}
			...
			<div class="flex flex-row justify-between">
				...
				>
				<h1 class="text-3xl font-extrabold text-right ">{new Date(timestamp*1000).toLocaleTimeString('it-IT', {hour: '2-digit', minute:'2-digit'})}<br><span class="font-light text-xl italic">Orario</span></h1>
			</div>
				<div class="mb-5">
